...
Limitations: only one property/value pair per call. group_by only works when a name filter is set. Returns at most \`limit\` rows per group (default 10, max 50). Numeric custom properties are stored as strings — for revenue use revenue.sum, which understands the Money type.`,
    {
...
Limitations: forcing granularity="hour" over a 90-day period produces hundreds of buckets and may be truncated server-side. Buckets with no matching events return zero (the series does not skip missing dates).`,
  ...
  server.tool(
    "funnels.create",
    `Create and immediately evaluate a conversion funnel. A funnel measures how many unique visitors complete an ordered sequence of steps. Returns step-by-step counts, per-step conversion rates (vs the previous step), and overall conversion (last step / first step). The funnel is saved by name and can be re-evaluated later or for different periods via funnels.get. Steps are either custom event names (e.g. "signup") or pageviews scoped to a path using "pageview:/path" syntax (e.g. "pageview:/pricing"). Requires Pro plan.
...
Limitations: between 2 and 10 steps; names ≤200 chars. Step order matters — once a session skips a step, it cannot complete later steps. Pageview steps match exact paths only (no wildcards). Funnel evaluation is per-session, not per-user across devices.`,
    {
      project_id: projectIdParam,
      name: z
        .string()
...
Limitations: keyword (substring) matching only — no semantic search, so synonyms ("webhook" vs "callback") will not match. The index is hand-maintained inside the MCP server and updates only when the server is redeployed. Returns an empty array if no matches; broaden the query rather than retrying with the same terms.`;
  ...
  const searchDocsHandler = async ({ query, limit }: { query: string; limit?: number }) => {
      const DOCS = [
        ...
        { url: "https://clamp.sh/docs/skills", title: "Analytics skills", desc: "Five model-invoked agent skills (analytics-profile-setup, analytics-diagnostic-method, traffic-change-diagnosis, channel-and-funnel-quality, metric-context-and-benchmarks). Calibrated benchmarks instead of generic averages." },
