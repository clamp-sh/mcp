...
const projectIdParam = z
  ...
  .optional()
  .describe(
    'Target project ID (e.g. "proj_abc123"). Required when the credential has access to multiple projects. If omitted and only one project is accessible, that project is used automatically. Call `list_projects` to discover available project IDs.',
  );
...
/** Supported dimensions for the generic `get_breakdown` tool. */
...
export function registerClampTools(server: McpServer, ctx: ClampToolContext): void {
  ...
  server.tool(
    "get_top_referrers",
    'Top traffic sources (referrer hostnames) ranked by unique visitors. Returns referrer_host (e.g. "google.com"), channel ("direct", "organic_search", "organic_social", "paid", "email", "referral"), visitors, and pageviews. **Use this for referrer_host totals.** For channel totals alone use get_breakdown with dimension="channel". For UTM sources use get_breakdown with dimension="utm_source".',
    { project_id: projectIdParam, period: periodParam, limit: limitParam, ...commonFilterShape },
    { readOnlyHint: true },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
  ...
  server.tool(
    "get_countries",
    'Visitor counts by country. Returns ISO 3166-1 alpha-2 codes (e.g. "US", "GB", "DE") with unique visitors and pageviews. Sorted by visitors descending. **Use this for country totals.** For sub-country geography use get_cities, or get_breakdown with dimension="region".',
    { project_id: projectIdParam, period: periodParam, limit: limitParam, ...commonFilterShape },
    { readOnlyHint: true },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
  ...
  server.tool(
    "get_cities",
    'Visitor counts by city, with the ISO country code alongside each row. Optionally filter to one country. Sorted by visitors descending. **Use this for city totals.** For country totals use get_countries. For state/region use get_breakdown with dimension="region".',
    {
  ...
  server.tool(
    "get_devices",
    'Visitor breakdown by device type, browser, browser_version, OS, or os_version. Set group_by to choose which. **Use this for any device/browser/OS question** (including versions). For geography see get_countries/get_cities; for pages see get_top_pages; for everything else see get_breakdown.',
    {
      project_id: projectIdParam,
      period: periodParam,
      limit: limitParam,
  ...
  server.tool(
    "list_alerts",
    "List all alerts for a project. Returns each alert's id, metric, condition, threshold, period, pathname, and created_at. Use before delete_alert to find the alert id, or to show the user what alerts are currently configured.",
    { project_id: projectIdParam },
    { readOnlyHint: true },
    async ({ project_id }) => {
      const p = resolveProject(project_id);
  ...
  server.tool(
    "delete_alert",
    "Delete an alert by its id. Call list_alerts first to find the id. Irreversible.",
    {
      project_id: projectIdParam,
      alert_id: z.string().min(1).describe("The alert id (UUID) to delete. Get this from list_alerts."),
    },
  ...
  server.tool(
    "get_breakdown",
    'Generic single-dimension breakdown of pageviews and unique visitors. **Prefer a specialized tool when one exists:** get_top_pages (pathname), get_top_referrers (referrer_host), get_countries (country), get_cities (city), get_devices (device_type/browser/os and their versions). Use get_breakdown for the remaining dimensions: entry_page, exit_page, region, channel, utm_source, utm_medium, utm_campaign, utm_content, utm_term.',
    {
      project_id: projectIdParam,
      dimension: z
  ...
  const searchDocsHandler = async ({ query, limit }: { query: string; limit?: number }) => {
      const DOCS = [
        ...
        { url: "https://clamp.sh/docs/concepts/revenue", title: "Revenue", desc: "Attach Money-typed properties and query revenue by source, country, campaign with get_revenue. Refunds, mixed currencies." },
        { url: "https://clamp.sh/docs/concepts/engagement", title: "Engagement and sessions", desc: "How visible time is measured, pageview_end beacon, bounce rate, get_page_engagement, get_session_paths." },
  ...
  server.tool("search_docs", searchDocsDescription, searchDocsSchema, { readOnlyHint: true }, searchDocsHandler);
  // Back-compat alias. Remove in next major.
  server.tool(
    "docs_search",
    `Deprecated: use search_docs. ${searchDocsDescription}`,
    searchDocsSchema,
    { readOnlyHint: true },
    searchDocsHandler,
  );
  ...
  const scopeHint =
    projects.length === 1
      ...
      : projects.length === 0
        ...
        : `Multiple projects available. If the user did not specify which one, ask them, or call list_projects to show the options. Pass the chosen \`project_id\` to every tool call.`;
