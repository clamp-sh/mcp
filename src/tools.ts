...
/** Supported dimensions for the `traffic.breakdown` tool. */
...
/**
 * Build a CallToolResult that satisfies both the legacy text-content shape
 * (`content: [{ type: "text", text: ... }]`) and the structured-content shape
 * required when `outputSchema` is declared. The serialized JSON in `content`
 * matches `structuredContent` exactly, per MCP spec.
 *
 * Per MCP spec, structuredContent must be an object — array-returning handlers
...
// can read the schema in tools/list to type-check responses ahead of calls.
//
// Per MCP spec, structuredContent must be an object — array-returning APIs
// wrap their payload at the handler level into a keyed object (rows, events,
// series, etc.) before calling out().
...
const trafficOverviewOutput = {
  ...
  comparison: z.object({
    pageviews: overviewMetricSchema,
    visitors: overviewMetricSchema,
    sessions: overviewMetricSchema,
    bounce_rate: overviewMetricSchema,
  }),
};
...
const observedEventSchema = z.object({
  name: z.string(),
  count: z.number(),
  properties: z.record(z.string(), z.array(observationSchema)),
});

const eventsObservedSchemaOutput = {
  period: z.string(),
...
const usersJourneyOutput = {
  anonymous_id: z.string(),
  sessions: z.array(journeySessionSchema),
};
...
const errorsGroupsOutput = {
  period: z.string(),
  sort_by: z.string(),
  groups: z.array(errorGroupSchema),
};

const errorsTimelineOutput = {
  period: z.string(),
  interval: z.string(),
  fingerprint: z.string().nullable(),
  series: z.array(z.object({ bucket: z.string(), count: z.number() })),
};

const breadcrumbEventSchema = z.object({
  timestamp: z.string(),
  name: z.string(),
  url: z.string().optional(),
  pathname: z.string().optional(),
...
const errorsContextOutput = {
  anonymous_id: z.string(),
  before_timestamp: z.string(),
  count: z.number(),
  events: z.array(breadcrumbEventSchema),
};

const trafficTimeseriesOutput = {
  series: z.array(z.object({ date: z.string(), count: z.number() })),
};
...
const trafficBreakdownOutput = {
  rows: z.array(breakdownRowSchema),
};

const livePageRowSchema = z.object({ name: z.string(), pageviews: z.number(), visitors: z.number() });
const liveNameVisitorsRowSchema = z.object({ name: z.string(), visitors: z.number() });

const trafficLiveOutput = {
  window_minutes: z.number(),
  visitors: z.number(),
  pageviews: z.number(),
  top_pages: z.array(livePageRowSchema),
  top_sources: z.array(liveNameVisitorsRowSchema),
  top_countries: z.array(liveNameVisitorsRowSchema),
};

const trafficCompareOutput = {
  metric: z.string(),
  a: z.object({ period: z.string(), value: z.number() }),
  b: z.object({ period: z.string(), value: z.number() }),
  absolute_delta: z.number(),
  pct_delta: z.string(),
};

const sessionPathRowSchema = z.object({
  entry_page: z.string(),
  exit_page: z.string(),
  sessions: z.number(),
  avg_pages: z.number(),
...
const pagesEngagementOutput = {
  pages: z.array(pageEngagementRowSchema).optional(),
...
const cohortDefinitionSchema = z.object({
  event: z.string(),
  period: z.string(),
  filter: z.object({ property: z.string(), value: z.string() }).optional(),
});
...
const funnelResultSchema = z.object({
  ...
  overall_conversion: z.string(),
  period: z.string(),
});

const funnelsCreateOutput = {
  id: z.string(),
  name: z.string(),
  steps: z.array(funnelStepSchema),
  overall_conversion: z.string(),
...
const funnelsListOutput = {
  funnels: z.array(funnelResultSchema),
};
...
export function registerClampTools(server: McpServer, ctx: ClampToolContext): void {
  ...
  // ─────────────────────────────────────────────────────────────────────
  // Every tool lives under a resource namespace (`resource.something`).
  // Three sub-conventions, picked by what the tool *is*:
  //
  //   1. CRUD on a managed resource (saved funnels, cohorts, alerts):
  //        `resource.list` / `resource.create` / `resource.delete`
  //      Examples: alerts.create, cohorts.list, funnels.list, errors.list,
  //                events.list, projects.list
  //
  //   2. Action verb on data (the operation is what matters):
  //        `resource.verb`
  //      Examples: revenue.sum, traffic.compare, traffic.breakdown,
  //                cohorts.compare, cohorts.retention, docs.search
  //
  //   3. Shaped read (the returned thing is what matters):
  //        `resource.thing`
  //      Examples: traffic.overview, traffic.timeseries, traffic.live,
  //                sessions.paths, pages.engagement, users.journey,
  //                events.observed_schema, errors.groups, errors.timeline,
  //                errors.context
  //
  // When adding a new tool, ask: am I doing CRUD, an action, or shaping a
  // read? Pick the matching pattern. Never ship a tool without a namespace
  // (no bare verbs like `breakdown` or bare nouns like `overview`).
  // ─────────────────────────────────────────────────────────────────────
...
Limitations: returns an empty array if the credential is org-scoped but has no projects yet. The plan field is the current billing plan, not a permission level.`,
    ...
    async () =>
      out({ projects: projects.map((p) => ({ id: p.id, name: p.name, plan: p.plan })) }),
  );
...
Limitations: bounce_rate and avg_duration are derived from the SDK's pageview_end beacon — for SDK <0.3 they return null. Custom date ranges must be in YYYY-MM-DD:YYYY-MM-DD format. Maximum range is 365 days.`,
      inputSchema: { project_id: projectIdParam, period: periodParam, ...commonFilterShape },
      outputSchema: trafficOverviewOutput,
      annotations: { readOnlyHint: true },
    },
...
Limitations: only one property/value pair per call. group_by only works when a name filter is set. Returns at most \`limit\` rows per group (default 10, max 50). For revenue use revenue.sum, which understands the Money type.`,
    ...
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
      if (isErr(p)) return p;
      const data = (await api(`/analytics/${p.projectId}/events${qs(rest)}`)) as unknown[];
      return out({ events: data });
    },
  );

  // ── Tool: events.observed_schema ───────────────────
  server.registerTool(
...
Limitations: events without any Money property contribute zero. If \`property\` is set, only that one Money key is summed; omitted, every Money property on matched events is included. Stripe-typed revenue (recommended) flows through server-side webhooks; client-only revenue is subject to ad-blocker loss. \`attribution_model="first_touch"\` only supports acquisition-dimension group_by (channel, referrer_host, utm_*); first-touch country / device / pathname are rejected because they're rarely what people actually mean by them. First-touch breakdowns are extremely sensitive to small samples — at single-digit paying-customer counts the per-channel rate is dominated by 1-2 users' acquisition history; check \`visitors\` before reading the rate, or pair with \`users.journey\` to validate against specific customers.`,
      ...
      attribution_model: z
        ...
        .optional()
        .describe(
          'Attribution model. "last_touch" (default) groups by the dimensions of the session where revenue fired. "first_touch" joins each revenue event with the visitor\'s earliest session and groups by that session\'s acquisition dim. First-touch only supports group_by in acquisition dimensions (channel, referrer_host, utm_*).',
        ),
      limit: limitParam,
        ...commonFilterShape,
      },
      outputSchema: revenueSumOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
...
Pairs with: \`revenue.sum(attribution_model="first_touch")\` to validate the aggregate first-touch read against specific high-value customers' actual paths.`,
      ...
      limit: z
        ...
        .max(200)
        .optional()
        .describe("Max sessions to return. Defaults to 50, max 200."),
      },
      outputSchema: usersJourneyOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
...
Pairs with: \`errors.groups\` (find a noisy fingerprint, then list its occurrences here); \`errors.context\` (drill from one error row into the events from the same session that led to it); \`users.journey\` (full multi-session view of a user who hit an error).`,
      ...
      message: z
        ...
        .max(1024)
        .optional()
        .describe('Filter by exact error message (matches the `error.message` property). Use a known message verbatim; substring matching is not supported.'),
      fingerprint: z
        .string()
        .max(64)
        .optional()
        .describe('Filter by server-computed fingerprint (16-char hex). Get fingerprints from errors.groups; same fingerprint groups all occurrences of the same bug.'),
      handled: z
        .enum(["true", "false"])
        .optional()
        .describe('Filter to handled (manual captureError calls) or unhandled (window.onerror / unhandledrejection auto-capture). Omit to return both.'),
      browser: z
        .string()
        .max(64)
        .optional()
        .describe('Filter by browser name (e.g. "Chrome", "Safari", "Firefox"). Case-sensitive; use the exact value the SDK reports.'),
      os: z
        .string()
        .max(64)
        .optional()
        .describe('Filter by OS name (e.g. "macOS", "Windows", "iOS", "Android").'),
      device_type: z
        .enum(["desktop", "mobile", "tablet", "bot"])
        .optional()
        .describe('Filter by device type. Use this to isolate mobile-only bugs.'),
      country: z
        .string()
        .length(2)
        .optional()
        .describe('Two-letter ISO country code (e.g. "US", "DE"). Useful when an error correlates with a CDN edge or a localization issue.'),
      },
      outputSchema: errorsListOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
  ...
  server.registerTool(
    "errors.groups",
    {
      description: `Errors deduplicated by server-computed fingerprint, with counts and first/last-seen timestamps. The triage view: which bugs are firing most often, affecting the most users, or showing up newly in the period.
...
Pairs with: \`errors.list\` (drill into a fingerprint to see individual occurrences with full context); \`errors.timeline\` (chart the rate of one fingerprint to spot regressions or post-deploy regressions); \`traffic.compare\` (correlate a count spike with a deploy or campaign).`,
      ...
      limit: limitParam,
      sort_by: z
        .enum(["count", "users_affected", "last_seen", "first_seen"])
        .optional()
        .describe('Order groups by total occurrences (default), unique users affected, most recent occurrence, or earliest occurrence. Sort is always descending.'),
      },
      outputSchema: errorsGroupsOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
  ...
  // ── Tool: errors.timeline ──────────────────────────
  server.registerTool(
    "errors.timeline",
    {
      description: `Error count over time, bucketed hourly or daily. Optionally scoped to one fingerprint. Use this to spot regressions ("when did this bug start firing?") or post-deploy spikes ("did the deploy at 14:00 break something?").
...
Pairs with: \`errors.groups\` (find which fingerprint is worth charting); \`traffic.timeseries\` (compare error count to traffic count to see if the rate is real or just a traffic surge); \`traffic.compare\` (week-over-week or before-after-deploy comparison).`,
      ...
      fingerprint: z
        ...
        .max(64)
        .optional()
        .describe('Optional fingerprint (16-char hex from errors.groups) to scope the chart to one bug. Omit for total error rate.'),
      interval: z
        .enum(["hour", "day"])
        .optional()
        .describe('Bucket size. Defaults to "hour" for periods ≤ 7 days, "day" otherwise.'),
      },
      outputSchema: errorsTimelineOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
  ...
  server.registerTool(
    "errors.context",
    {
      description: `Breadcrumbs leading to one error: the events from the same session, before the error timestamp, in chronological order. The "what was the user doing right before this broke" view that turns a stack trace into a story.
...
Pairs with: \`errors.list\` (source of the anonymous_id and timestamp pair); \`users.journey\` (multi-session view of the same user, when one-session breadcrumbs aren't enough).`,
      ...
      anonymous_id: z
        ...
        .min(1)
        .max(128)
        .describe('The visitor whose session leading up to the error you want to inspect. Pull this from an errors.list row.'),
      before_timestamp: z
        .string()
        .describe('ISO 8601 timestamp of the error itself. Returns events strictly earlier than this from the same session.'),
      limit: z
        .coerce
        .number()
        .int()
        ...
        .max(100)
        .optional()
        .describe('Max events to return. Defaults to 20, max 100. Increase only if a longer breadcrumb is needed; agents typically read the most recent ones first.'),
      },
      outputSchema: errorsContextOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
...
Limitations: forcing granularity="hour" over a 90-day period produces hundreds of buckets and may be truncated server-side. Buckets with no matching events return zero (the series does not skip missing dates).`,
      ...
      outputSchema: trafficTimeseriesOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
  ...
  server.registerTool(
    "funnels.create",
    {
      description: `Create and immediately evaluate a conversion funnel. A funnel measures how many unique visitors complete an ordered sequence of steps. Returns step-by-step counts, per-step conversion rates (vs the previous step), and overall conversion (last step / first step). The funnel is saved by name and can be re-evaluated later or for different periods via funnels.list.
...
Limitations: between 2 and 10 steps; step strings ≤500 chars; names ≤200 chars. Step order matters — once a session skips a step, it cannot complete later steps. Pageview pathnames match exact strings only (no wildcards). Predicate keys must be snake_case; string values may not contain ']' or '['. Number predicates require a finite value; boolean predicates require true|false|1|0. Funnel evaluation is per-session, not per-user across devices.`,
  ...
  server.registerTool(
    "funnels.list",
    {
      description: `Retrieve and re-evaluate a previously created funnel against current data for the specified period. Without a \`name\`, lists all funnels saved for the project. With a \`name\`, returns the same step-by-step counts and conversion rates as funnels.create, recomputed for the requested period and any cohort filters. Cohort filters (channel, country, device_type, utm_*) let you compare conversion across segments — e.g. mobile users from the US who came via organic search.
...
Limitations: returns 404 if no funnel exists by that name — call funnels.list with no name first to enumerate. Cohort filters apply at the session level, not retroactively per step. Funnel definitions are immutable after creation (re-create with a new name to change steps).`,
      ...
      outputSchema: funnelsListOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
  ...
  server.registerTool(
    "cohorts.create",
    {
      description: `Create and persist a named cohort — a group of visitors defined by an event in a period, optionally narrowed by a property filter. The cohort is stored by name; subsequent retention/compare queries reference it without re-specifying the definition. Membership is recomputed at query time, not materialised, so the same cohort always reflects current data. Requires Pro plan.
...
Pairs with: \`cohorts.retention\` to read the curve once the cohort is created; \`cohorts.compare\` for two-cohort side-by-side reads; \`cohorts.list\` to discover existing cohort handles.`,
      ...
      name: z
        ...
        .min(1)
        .max(200)
        .describe('Cohort name. Lowercase alphanumeric, hyphens, underscores. Used as the handle for retention / compare / delete.'),
      definition: z
        .object({
          event: z
            .string()
            .min(1)
            .max(200)
            .describe("Event name members must have fired (e.g. 'signup_completed', 'purchase')."),
          period: z
            .string()
            .min(1)
            .max(64)
            .describe('Period the event must fall within. ISO range "YYYY-MM-DD:YYYY-MM-DD" or a preset ("7d", "30d", "today", "yesterday").'),
          filter: z
            .object({
            ...
            .optional()
            .describe('Optional property filter. Narrows the cohort to events where the property equals this value.'),
        })
    ...
    async ({ project_id, name, definition }) => {
      const p = resolveProject(project_id);
      if (isErr(p)) return p;
      const data = await api(`/analytics/${p.projectId}/cohorts`, {
        method: "POST",
  ...
  server.registerTool(
    "cohorts.list",
    {
      description: `List saved cohorts for the project. With \`name\`, returns one cohort with its current size; without, returns every cohort with their definitions. Use to discover cohort handles before calling retention or compare.
...
Limitations: \`size\` is recomputed at fetch time and reflects current data — a cohort defined as "last 30d" returns a moving size as new events land.`,
      inputSchema: {
        project_id: projectIdParam,
      name: z.string().optional().describe("Cohort name to fetch one. Omit to list all."),
      },
    ...
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
      if (isErr(p)) return p;
      const data = await api(`/analytics/${p.projectId}/cohorts${qs(rest)}`);
      const cohorts = Array.isArray(data) ? data : [data];
      return out({ cohorts });
    },
  );

  // ── Tool: cohorts.retention ─────────────────────────
  server.registerTool(
    "cohorts.retention",
    {
      description: `Compute the retention curve for a saved cohort. For each requested window (e.g. day 1, 7, 14, 30 after the cohort's anchor event), returns how many cohort members fired any non-pageview-end event in that day's window, and the rate vs the cohort size.
...
Pairs with: \`cohorts.compare\` for two-cohort side-by-side; \`cohorts.list\` to discover available cohort names.`,
      ...
      periods: z
        .string()
        .optional()
        .describe('Comma-separated retention windows. Each is `<n>d` or `<n>w` (e.g. "1d,7d,14d,30d" or "1w,2w,4w,8w"). Defaults to "1d,7d,14d,30d".'),
      },
      outputSchema: cohortsRetentionOutput,
      annotations: { readOnlyHint: true },
    },
...
Limitations: only two cohorts at a time. The same retention windows are applied to both — there's no way to use different windows per side. Sample-size caveats apply per cohort; check both \`size\` values before reading the rate delta.`,
  ...
  server.registerTool(
    "cohorts.delete",
    {
      description: `Delete a saved cohort by name. Irreversible — the agent should confirm intent with the user before calling this. The underlying event data isn't touched; only the cohort definition row is removed.
...
Limitations: irreversible. Returns 404 if the cohort doesn't exist. Definition is not returned before deletion — capture it from cohorts.list first if you may need to recreate it.`,
      inputSchema: {
        project_id: projectIdParam,
      name: z.string().min(1).describe("Cohort name to delete."),
      },
    ...
    async ({ project_id, name }) => {
      ...
      if (isErr(p)) return p;
      return out(
        await api(`/analytics/${p.projectId}/cohorts/${encodeURIComponent(name)}`, {
          method: "DELETE",
        }),
      );
    },
...
Limitations: evaluation is on MCP session connect, not background — there are no push notifications, emails, or webhooks. Threshold for above/below is the absolute metric value; for drops_by/increases_by it is the percentage change vs the previous period of the same length. One alert per call — create multiple alerts for multiple conditions.`,
  ...
  server.registerTool(
    "alerts.list",
    {
      description: `List all alerts configured for a project. Returns each alert's id (UUID), metric, condition, threshold, period, optional pathname scope, and created_at timestamp. Use before alerts.delete to find the id you want to remove, or to show the user every monitor currently active. Returns an empty array if no alerts have been created yet.
...
Limitations: alert state (currently firing vs not) is not included — that surfaces only when the alert is re-evaluated on MCP session connect. There is no pagination; projects with many alerts return all of them in one response.`,
    ...
    async ({ project_id }) => {
      const p = resolveProject(project_id);
      if (isErr(p)) return p;
      const data = (await api(`/analytics/${p.projectId}/alerts`)) as unknown[];
  ...
  server.registerTool(
    "alerts.delete",
    {
      description: `Delete an alert by its id. Find the id by calling alerts.list first. The deletion is irreversible — there is no soft-delete or undo, and the agent should confirm intent with the user before calling this on a non-trivial alert. Returns 404 if no alert with that id exists for the project.
...
Limitations: aggregates pageview events only — for custom event breakdowns use events.list with \`group_by\`. The \`name\` column is the raw stored value (lowercase ISO codes for country, exact pathname strings including trailing slash). Per-page time-on-page or bounce rate is not included here — use pages.engagement for that.`,
      ...
      outputSchema: trafficBreakdownOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
  ...
  server.registerTool(
    "traffic.compare",
    {
      description: `Compare one metric across two arbitrary periods side-by-side. Returns both period values plus absolute delta and percentage delta. Periods do not need to be the same length — the percentage delta normalizes by ratio so longer/shorter comparisons remain meaningful. Use when the user asks month-over-month, before-vs-after-launch, or "how does this week compare to last".
...
Limitations: one metric per call — for multi-metric comparison either call repeatedly or use traffic.overview (which always includes the previous period). Period strings must be a preset ("today", "yesterday", "7d", "30d", "90d") or a YYYY-MM-DD:YYYY-MM-DD range; relative phrases like "last quarter" are not parsed.`,
      ...
      outputSchema: trafficCompareOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
  ...
  server.registerTool(
    "sessions.paths",
    {
      description: `Aggregate (entry_page → exit_page) session pairs. Returns the top pairs with how many sessions followed each path, average pages per session, average duration in seconds, and a bounce flag. Use to answer "what do visitors do after landing on /pricing", "where do sessions end", or "which entry pages lead to the deepest engagement". Aggregate-only — no per-user traces, no full pageview chains, no individual session reconstruction.
...
Limitations: shows entry and exit only, not the full pageview chain in between (use events.list for granular event analysis). min_pages=1 (default) includes single-page sessions which always show as bounces; set min_pages=2 to exclude them. Sessions ending without a pageview_end beacon (e.g. browser crash) may have null durations.`,
      ...
      outputSchema: sessionsPathsOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
...
Limitations: avg_engagement_seconds is null for pages without pageview_end data (SDK <0.3 or pages closed during navigation). view="sections" requires the section-views SDK extension installed (see /docs/sdk/extensions/section-views) and only counts elements with the data-clamp-section attribute — pages with no instrumented sections return []. view="sections" without \`pathname\` returns 400.`,
      ...
      outputSchema: pagesEngagementOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, ...rest }) => {
      const p = resolveProject(project_id);
...
Limitations: ingestion lag is ~30 seconds, so "live" is approximate. Visitor count is unique anonymous_ids in the window, not active sessions. For historical questions ("who visited last week"), use traffic.overview instead.`,
      ...
      outputSchema: trafficLiveOutput,
      annotations: { readOnlyHint: true },
    },
    async ({ project_id, window_minutes }) => {
      const p = resolveProject(project_id);
      if (isErr(p)) return p;
      return out(await api(`/analytics/${p.projectId}/realtime${qs({ window_minutes })}`));
    },
  );
...
Limitations: keyword (substring) matching only — no semantic search, so synonyms ("webhook" vs "callback") will not match. The index is hand-maintained inside the MCP server and updates only when the server is redeployed. Returns an empty array if no matches; broaden the query rather than retrying with the same terms.`;
  ...
  const searchDocsHandler = async ({ query, limit }: { query: string; limit?: number }) => {
      const DOCS = [
        ...
        { url: "https://clamp.sh/docs/sdk/server", title: "SDK server-side (Node)", desc: "Track from Node.js, Cloudflare Workers, Vercel Edge, Deno Deploy, Bun. Link browser and server events with an anonymous ID. Authoritative revenue from webhooks. Includes a Stripe-revenue agent prompt and a captureError section." },
        { url: "https://clamp.sh/docs/sdk/server/python", title: "Python SDK", desc: "Server-side analytics for Django, FastAPI, Flask, Celery. Same wire shape as the Node SDK. Includes captureError for exception tracking and per-framework integration patterns." },
        { url: "https://clamp.sh/docs/sdk/server/php", title: "PHP SDK", desc: "Server-side analytics for Laravel, Symfony, WordPress. PHP 8.1+. Includes Analytics::captureError for exceptions and per-framework integration patterns." },
        { url: "https://clamp.sh/docs/sdk/server/go", title: "Go SDK", desc: "Server-side analytics for net/http, gin, echo, chi. Pure stdlib, no external deps. Includes clamp.CaptureError for exceptions and per-framework integration patterns." },
        { url: "https://clamp.sh/docs/sdk/server/ruby", title: "Ruby SDK", desc: "Server-side analytics for Rails, Sinatra, Sidekiq. Ruby 3.0+, stdlib only. Includes capture_error for exceptions and per-framework integration patterns." },
        { url: "https://clamp.sh/docs/sdk/extensions", title: "SDK extensions overview", desc: "Catalog of error capture, outbound links, downloads, 404 detection, data attributes, web vitals, section views. Path exclusions and debug mode. Each extension has its own subpage." },
        { url: "https://clamp.sh/docs/sdk/extensions/errors", title: "Error capture extension", desc: "Capture exceptions and unhandled rejections as $error events. Browser auto-capture via { captureErrors: true }, plus per-language captureError on every server SDK. Read via the errors.* MCP tools." },
      ...
      return out({ query, results: scored });
  };
  server.registerTool(
    "docs.search",
    {
...
3. Call **traffic.breakdown** with dimension="referrer_host", period="7d", limit=10 for traffic sources.
4. Call **traffic.breakdown** with dimension="country", period="7d", limit=10 for geographic distribution.
...
2. Call **traffic.breakdown** with dimension="referrer_host", period="${period || "7d"}" — see if a specific source drove the change. Compare organic_search vs direct vs referral vs paid vs social (channel comes back joined per row).
3. Call **traffic.breakdown** with dimension="country", period="${period || "7d"}" — check if the change is concentrated in specific countries.
4. Call **traffic.breakdown** with dimension="device_type", period="${period || "7d"}" — check if it's a specific device type (mobile spike could mean social traffic).
...
1. Call **funnels.list** with period="30d"${funnel_name ? ` and name="${funnel_name}"` : ""} to get funnel step-by-step conversion data.
2. Call **events.list** with period="30d" to see all custom event volumes.
3. For the funnel with the biggest drop-off, call **funnels.list** with cohort filters to segment:
...
1. Call **traffic.breakdown** with dimension="referrer_host", period="${period || "30d"}", limit=20 — get the full referrer picture (channel comes back joined per row).
...
3. Call **traffic.breakdown** with dimension="referrer_host", period="${period || "30d"}", pathname="${pathname}" — where does this page's traffic come from specifically?
4. Call **traffic.breakdown** with dimension="country", period="${period || "30d"}" — context for who's visiting.
5. Call **traffic.breakdown** with dimension="device_type", period="${period || "30d"}" — any mobile/desktop split issues for this page?
