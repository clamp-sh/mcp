...
export interface ClampToolContext {
  /**
   * HTTP caller for the Clamp analytics API. Paths are relative and may
   * contain a literal `{pid}` placeholder that gets substituted with the
   * resolved projectId. The caller should add auth headers and throw
   * descriptive errors on non-2xx responses.
   */
  api: <T = unknown>(path: string, options?: RequestInit) => Promise<T>;
  projectId: string;
  projectName: string;
  /** Current plan ("free" | "pro" | "growth"). Surfaced in prompts. */
  plan: string;
}
...
function responseStyle(plan: string): string {
  return `
...
export function registerClampTools(server: McpServer, ctx: ClampToolContext): void {
  const { api, projectId, projectName, plan } = ctx;
  const subst = (path: string) => path.replace("{pid}", projectId);
  ...
  server.tool(
    ...
    { period: periodParam, ...commonFilterShape },
    { readOnlyHint: true },
    async (args) => json(await api(subst(`/analytics/{pid}/overview${qs(args)}`))),
  );
  ...
  server.tool(
    ...
    { period: periodParam, limit: limitParam, ...commonFilterShape },
    { readOnlyHint: true },
    async (args) => json(await api(subst(`/analytics/{pid}/top-pages${qs(args)}`))),
  );
  ...
  server.tool(
    ...
    { period: periodParam, limit: limitParam, ...commonFilterShape },
    { readOnlyHint: true },
    async (args) => json(await api(subst(`/analytics/{pid}/top-referrers${qs(args)}`))),
  );
  ...
  server.tool(
    ...
    { period: periodParam, limit: limitParam, ...commonFilterShape },
    { readOnlyHint: true },
    async (args) => json(await api(subst(`/analytics/{pid}/countries${qs(args)}`))),
  );
  ...
  server.tool(
    ...
    { readOnlyHint: true },
    async ({ period, limit, country }) => json(await api(subst(`/analytics/{pid}/cities${qs({ period, limit, country })}`))),
  );

  // ── Tool: get_devices ──────────────────────────────
  server.tool(
    "get_devices",
    "Get visitor breakdown by device type, browser, browser_version, OS, or os_version. Set group_by to choose the dimension. Returns the dimension name with visitor and pageview counts.",
    {
      period: periodParam,
      limit: limitParam,
      group_by: z
    ...
    { readOnlyHint: true },
    async (args) => json(await api(subst(`/analytics/{pid}/devices${qs(args)}`))),
  );
  ...
  server.tool(
    ...
    { readOnlyHint: true },
    async (args) => json(await api(subst(`/analytics/{pid}/events${qs(args)}`))),
  );
  ...
  server.tool(
    ...
    { readOnlyHint: true },
    async (args) => json(await api(subst(`/analytics/{pid}/timeseries${qs(args)}`))),
  );
  ...
  server.tool(
    ...
    async ({ name, steps }) => {
      const data = await api(subst(`/analytics/{pid}/funnels`), {
        method: "POST",
  ...
  server.tool(
    ...
    { readOnlyHint: true },
    async (args) => json(await api(subst(`/analytics/{pid}/funnels${qs(args)}`))),
  );
  ...
  server.tool(
    ...
    async ({ metric, condition, threshold, period, pathname }) => {
      const data = await api(subst(`/analytics/{pid}/alerts`), {
        method: "POST",
  ...
  server.tool(
    "get_breakdown",
    "Get a single-dimension breakdown of pageviews and unique visitors. Dimensions include pathname, entry_page, exit_page, referrer_host, channel, country, region, city, device_type, browser, browser_version, os, os_version, and all UTM params (utm_source, utm_medium, utm_campaign, utm_content, utm_term). Use this instead of calling separate tools when you need dimensions like region, browser_version, or UTM medium/content/term.",
    {
      dimension: z
    ...
    { readOnlyHint: true },
    async (args) => json(await api(subst(`/analytics/{pid}/breakdown${qs(args)}`))),
  );
  ...
  server.tool(
    ...
    { readOnlyHint: true },
    async (args) => json(await api(subst(`/analytics/{pid}/compare${qs(args)}`))),
  );
  ...
  server.tool(
    ...
    async ({ window_minutes }) =>
      json(await api(subst(`/analytics/{pid}/realtime${qs({ window_minutes })}`))),
  );
  ...
  server.tool(
    ...
    async ({ query, limit }) => {
      const DOCS = [
        ...
        { url: "https://clamp.sh/docs/sdk", title: "SDK reference", desc: "clamp.track, clamp.identify, clamp.page, flush, config options." },
        { url: "https://clamp.sh/docs/events", title: "Custom events", desc: "Track signups, clicks, conversions. Event properties and naming." },
        { url: "https://clamp.sh/docs/funnels", title: "Funnels", desc: "Create and evaluate multi-step conversion funnels. Pro plan." },
        { url: "https://clamp.sh/docs/mcp", title: "MCP server", desc: "Install and configure the Clamp MCP in Claude, Cursor, VS Code, Windsurf. Full tool reference." },
  ...
  // ── Prompts ────────────────────────────────────────
  const style = responseStyle(plan);
...
Generate a comprehensive weekly report for project ${projectName} (${projectId}). Follow these steps:
...
Investigate what's driving traffic changes for project ${projectName} (${projectId}) over ${period || "7d"}. Follow this diagnostic sequence:
...
Analyze conversion performance for project ${projectName} (${projectId}). Steps:
...
Analyze traffic quality by channel for project ${projectName} (${projectId}) over ${period || "30d"}. Steps:
...
Analyze the performance of page "${pathname}" for project ${projectName} (${projectId}) over ${period || "30d"}. Steps:
