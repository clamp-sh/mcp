...
Limitations: between 2 and 10 steps; step strings ≤500 chars; names ≤200 chars. Step order matters — once a session skips a step, it cannot complete later steps. Pageview pathnames match exact strings only (no wildcards). Predicate keys must be snake_case; values may not contain ']' or '['. Funnel evaluation is per-session, not per-user across devices.`,
    {
      project_id: projectIdParam,
      name: z
        .string()
