...
server.tool(
  ...
  async ({ period, pathname, utm_source, utm_campaign, referrer_host }) => {
    const data = await api(
      `/analytics/${PROJECT_ID}/overview${qs({ period, pathname, utm_source, utm_campaign, referrer_host })}`,
    );
...
server.tool(
  ...
  async ({ period, limit, referrer_host }) => {
    const data = await api(
      `/analytics/${PROJECT_ID}/top-pages${qs({ period, limit, referrer_host })}`,
    );
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
  },
);
...
server.tool(
  ...
  async ({ period, limit, name, property, value, group_by, utm_source, utm_campaign, referrer_host }) => {
    const data = await api(
      `/analytics/${PROJECT_ID}/events${qs({ period, limit, name, property, value, group_by, utm_source, utm_campaign, referrer_host })}`,
    );
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
  },
);
...
server.tool(
  ...
  async ({ period, event, granularity, property, value, referrer_host }) => {
    const data = await api(
      `/analytics/${PROJECT_ID}/timeseries${qs({ period, event, granularity, property, value, referrer_host })}`,
    );
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
  },
);
...
server.tool(
  ...
  async ({ name, period, country, channel, device_type, utm_source, utm_campaign, pathname, referrer_host }) => {
    const data = await api(
      `/analytics/${PROJECT_ID}/funnels${qs({ name, period, country, channel, device_type, utm_source, utm_campaign, pathname, referrer_host })}`,
    );
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
  },
);
