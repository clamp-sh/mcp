...
| `get_cities` | Visitors by city. Optionally filter by country. |
| `get_devices` | Breakdown by device type, browser, or OS. |
| `get_events` | Custom event counts with property filtering and grouping. |
| `get_revenue` | Sum revenue from Money-typed event properties. Split by currency, optionally grouped by referrer_host, country, channel, UTM, etc. |
| `get_session_paths` | Aggregate session paths: top entry → exit pairs with pages per session and duration. |
| `get_page_engagement` | Per-page engagement seconds and bounce rate. |
