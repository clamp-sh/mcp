...
- [`5e2e8eb`](https://github.com/clamp-sh/clamp/commit/5e2e8ebc6806c59d06ae693a8fdf623c1be00a11) Thanks [@sbj-o](https://github.com/sbj-o)! - All 27 MCP tools now declare an `outputSchema` for typed response validation, per MCP protocol revision 2025-06-18. Each tool's `structuredContent` is validated against its schema by the SDK, and clients reading `tools/list` can type-check responses ahead of calls.

  **Breaking change:** tools whose underlying API returns an array now wrap the array in a meaningful key (per MCP spec, `structuredContent` must be an object). The change affects `traffic.timeseries`, `traffic.breakdown`, `events.list`, `revenue.sum`, `sessions.paths`, `pages.engagement`, `alerts.list`, `funnels.list`, `cohorts.list`. Both the text content and `structuredContent` reflect the new wrapped shape.

  Wrap keys per tool: `traffic.timeseries → series`, `traffic.breakdown → rows`, `events.list → events`, `revenue.sum → rows`, `sessions.paths → paths`, `pages.engagement → pages` (or `sections` when `view="sections"`), `alerts.list → alerts`, `funnels.list → funnels`, `cohorts.list → cohorts`.

  Migration: callers parsing the JSON text of these responses should switch from `data` to `data.<wrap_key>`. Tools returning objects (`traffic.overview`, `traffic.compare`, `traffic.live`, `users.journey`, `errors.*`, `funnels.create`, `cohorts.create`, `cohorts.retention`, `cohorts.compare`, `cohorts.delete`, `alerts.create`, `alerts.delete`, `events.observed_schema`, `projects.list`, `docs.search`) are unchanged.

  Internal: migrated from the deprecated `server.tool()` legacy API to `server.registerTool()` with config-object form.

## 3.0.2

### Patch Changes

- [`7161f84`](https://github.com/clamp-sh/clamp/commit/7161f846cebd714f8d50eb8fc74318023ea22298) Thanks [@sbj-o](https://github.com/sbj-o)! - Add explicit `"types": ["node"]` to the package tsconfig so the DTS build resolves `process` and other Node globals in environments where `@types/node` isn't auto-discovered (e.g. unhoisted pnpm installs in some Docker build environments).

## 3.0.1

### Patch Changes

- [#31](https://github.com/clamp-sh/clamp/pull/31) [`7000448`](https://github.com/clamp-sh/clamp/commit/70004486db45a6640811363c369183597291ad0b) Thanks [@sbj-o](https://github.com/sbj-o)! - `docs.search` index updated to reflect the docs reorganisation.

  - Polyglot SDK pages (`/docs/sdk/server/python`, `/docs/sdk/server/php`, `/docs/sdk/server/go`, `/docs/sdk/server/ruby`) now have entries; previously only the Node page was indexed and the URLs were flat under `/docs/sdk/<lang>`.
  - Added the missing `/docs/sdk/extensions/errors` entry so error-capture queries surface the right page.
  - Refreshed the `/docs/sdk/server` and `/docs/sdk/extensions` descriptions to mention error capture.
